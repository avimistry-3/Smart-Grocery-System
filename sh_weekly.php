...
<body>
	<?php
		...
		{
			$id=$_REQUEST['del'];
			$delt="delete from weekly where week_id='".$id."'";
			$exec=$cnc->query($delt);
			if($exec)
			{
	?>
				<script type="text/javascript">
					alert("Deleted");
					window.location.assign("sh_weekly.php");
				</script>
	<?php			
			}
			else
	...
	<form  method="POST" action="sh_weekly.php">
	<table border="1" id="x1" cellpadding="5px" cellspacing="5px" style="width: 800px;">
		...
		<?php
			...
			$ii=$v->us_id;
			$sql=" select * from weekly where us_id='".$ii."'";
		...
		?>
				<tr>
					...
					<td> <a href="sh_weekly.php?del=<?php echo $value->week_id; ?>"> Delete </a></td>
					<td> <a href="ed_weekly.php?edt=<?php echo $value->week_id; ?>"> Edit </a></td>
		 		</tr>
		<?php
		 	}
		?>
